...
def visualize_fps_and_surface(h5_file, tif_file, label, subset_index=0):
    """
    ...
    o3d.visualization.draw_geometries(
        ...
        point_show_normal=False,  # 显示法向量
        mesh_show_wireframe=True,  # 显示网格线框
        mesh_show_back_face=True,  # 显示反向面
        window_name=f"FPS and Original Surface of label {label} subset {subset_index}"
    )
    # # 法二：下面的代码是点云来显示颗粒
    # data = imread(tif_file)
    # data = np.transpose(data, (2, 1, 0)).copy()
...
# 使用示例
label = 19000  # 替换为需要对比的标签
subset_index = 0  # 替换为需要对比的子集索引（如果有多个子集）
tif_file = "I:/avizo-data/load1_ai_label.tif"  # 替换为实际的 .tif 文件路径
h5_file = "I:/data/h5data/load1_ai/all_test_fps_subsets.h5" # 替换为实际的 h5 文件路径

visualize_fps_and_surface(h5_file, tif_file, label, subset_index)
