...
def process_candidates_final_score(
    ...
    # )  # debug时暂时串行
    parallel_results = [process_one(p, round_idx) for p in pending_particles]
    results = []
    skipped_pending = []
...
def main():
    ...
    parser.add_argument('--info', required=True, help="简单描述修改的参数类型")
    ...
    parser.add_argument("--n_rounds", type=int, default=3, help="总共迭代轮数（含第2轮及以后）")
    ...
    if args.output_dir is None:
        args.output_dir = f"/share/home/202321008879/data/track_results/comparison/{args.def_type}to{args.ref_type}/{args.info}"
    ...
    if args.radius is None:
        args.radius = 4 * args.d50  # 搜索候选颗粒时用的范围，现在load15改为4d50
    ...
    # 保存第二轮
    output_file2 = os.path.join(args.output_dir, f'matchbyanchor_{args.info}2.csv')
    # 第二轮无需changed列
    ...
    # 保存最后一轮
    output_file_last = os.path.join(args.output_dir, f'matchbyanchor_{args.info}{args.n_rounds}.csv')
    output_file_prev = os.path.join(args.output_dir, f'matchbyanchor_{args.info}{args.n_rounds-1}.csv')
