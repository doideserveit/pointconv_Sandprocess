...
def num_anchor(pending_particles, anchor_kdtree, def_mapping, radius, all_particles_kdtree):
    """
    计算不同半径范围内的锚点数和所有颗粒数。
    """
    results = []
    for p in pending_particles:
        pending_centroid = def_mapping[p]['corrected_centroid(physical)']
        # 锚点数量
        idx_small = anchor_kdtree.query_ball_point(pending_centroid, r=radius)
        idx_large = anchor_kdtree.query_ball_point(pending_centroid, r=2 * radius)
        count_small = len(idx_small)
        count_large = len(idx_large)
        # 所有颗粒数量
        all_idx_small = all_particles_kdtree.query_ball_point(pending_centroid, r=radius)
        all_idx_large = all_particles_kdtree.query_ball_point(pending_centroid, r=2 * radius)
        all_count_small = len(all_idx_small)
        all_count_large = len(all_idx_large)
        results.append({
            'pending': p,
            'count_2d50': count_small,
            'count_4d50': count_large,
            'all_count_2d50': all_count_small,
            'all_count_4d50': all_count_large,
        })
    return results
...
def main():
    ...
    parser.add_argument('--ref_npz', default='/share/home/202321008879/data/sand_propertities/origin_ai/origin_particle_properties.npz', 
                        # /share/home/202321008879/data/sand_propertities/originnew_labbotm1k_particle_properties/originnew_labbotm1k_particle_properties.npz
    ...
    parser.add_argument('--output_dir', default='/share/home/202321008879/data/track_results/load1toorigin/matchbyanchor', 
                        help="输出文件路径")
    parser.add_argument('--predictions_file', default='/share/home/202321008879/data/track_results/load1toorigin/load1_ai_pred.npz',      
                        #/share/home/202321008879/data/track_results/load1_selpartooriginnew_labbotm1k/predictions.npz
                        help="预测结果 npz 文件")
    parser.add_argument('--init_match_file', default='/share/home/202321008879/data/track_results/load1toorigin/load1toorigin_initmatch.npz')
                        #/share/home/202321008879/data/track_results/load1_selpartooriginnew_labbotm1k/load1_selpartooriginnew_labbotm1k_initmatch.npz
    ...
    parser.add_argument("--K", type=int, default=5, help="补齐的锚点个数")
    ...
    # 构建所有颗粒 KD-Tree
    all_centroids = np.array([v['corrected_centroid(physical)'] for v in def_mapping.values()])
    all_particles_kdtree = KDTree(all_centroids)

    # 处理候选颗粒并计算最终分数
    results = num_anchor(pending_particles, kdtree, def_mapping, args.radius, all_particles_kdtree)
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "pending_label", "count_2d50", "count_4d50", "all_count_2d50", "all_count_4d50"
        ])
        for result in results:
            writer.writerow([
                ...
                result["count_2d50"],
                result["count_4d50"],
                result["all_count_2d50"],
                result["all_count_4d50"],
            ])
