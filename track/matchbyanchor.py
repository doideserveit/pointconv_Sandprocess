...
def get_candidate_particles(
    ...
    def process_one(pending_particle):
        ...
        for idx in idxs:
            ...
            if ref_lab in anchor_ref_labels:
                continue
...
def process_candidates_final_score(
    ...
    """
    def process_one(p, round_idx=round_idx):
...
def merge_anchors_with_matched(anchor_data, matched_dict, def_mapping, ref_mapping):
    """
    合并锚点和已匹配颗粒，返回新的anchor_data字典。
    matched_dict: {pending_label: candidate_label}
    """
    new_anchor = dict(anchor_data)
    for p, c in matched_dict.items():
...
def save_match_results_csv(results, output_file, best2=None):
    """
    保存匹配结果到csv。changed列只与第2轮比较。
    """
    with open(output_file, 'w', newline='') as f:
        ...
        for result in results:
            ...
            if result["matched"]:
                changed_flag = (best2 is not None and best2.get(result["pending"]) != result["candidate"])
            else:
                changed_flag = ""
            row.append(changed_flag)
            writer.writerow(row)
    print(f"匹配结果已保存到 {output_file}")

def save_skipped_pending(skipped_pending, output_file):
    with open(output_file, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=["pending", "anchor_count", "candidate_count"])
        writer.writeheader()
        for row in skipped_pending:
            writer.writerow(row)

def multi_round_matching(
    n_rounds, pending_particles, candidate_info, anchor_data, def_mapping, ref_mapping, args, weightUD, weightLD, results_round2, skipped_pending2
):
    """
    多轮迭代匹配流程，返回每一轮的结果和skipped_pending。
    每一轮输出与上一轮相比发生变化的颗粒数（不统计上一轮跳过的颗粒）。
    """
    round_results = []
    round_skipped = []
    anchor_and_matched = anchor_data
    # 第二轮已匹配的pending集合（未被跳过的）
    prev_valid = set(r['pending'] for r in results_round2 if r['matched'])
    prev_best = {r['pending']: r['candidate'] for r in results_round2 if r['matched']}
    prev_skipped = set(item['pending'] for item in skipped_pending2)
    for round_idx in range(3, n_rounds + 1):
        print(f"========== 第{round_idx}轮：最近{args.K}个已匹配颗粒 ==========")
        kdtree, anchor_labels = build_kdtree(anchor_and_matched)
        print(f"构建KDTree完成，锚点个数：{len(anchor_labels)}")
        results, skipped_pending = process_candidates_final_score(
        ...
        matched_dict = {r['pending']: r['candidate'] for r in results if r['matched']}
        # 统计与上一轮相比发生变化的颗粒数（只统计上一轮有匹配的pending，且上一轮未被跳过）
        changed_count = sum(
            1 for k, v in matched_dict.items()
            if k in prev_valid and prev_best.get(k) != v
        )
        print(f"第{round_idx}轮匹配完成，已匹配颗粒数：{len(matched_dict)}，与上一轮相比发生变化的颗粒数：{changed_count}")
        print(f"第{round_idx}轮锚点不足pending数：{len(skipped_pending)}")
        round_results.append(results)
        round_skipped.append(skipped_pending)
        # 合并锚点和本轮已匹配
        anchor_and_matched = merge_anchors_with_matched(anchor_and_matched, matched_dict, def_mapping, ref_mapping)
        # 更新本轮valid/skipped集合
        prev_valid = set(matched_dict.keys())
        prev_best = matched_dict
        prev_skipped = set(item['pending'] for item in skipped_pending)
    return round_results, round_skipped
...
def main():
    ...
    parser.add_argument("--multiple", type=int, default=2, help="候选颗粒搜索半径（默认2×d50）")

    parser.add_argument('--w', default=0, type=float, help="UD和LD的权重，0表示只用LD匹配，0.5表示UD和LD各占一半，0.75表示使用按照应变值计算的UD和LD权重")
    ...
    parser.add_argument("--n_rounds", type=int, default=10, help="总共迭代轮数（含初始匹配及第2轮）")
    ...
    if args.output_dir is None:
        args.output_dir = f"/share/home/202321008879/data/track_results/{args.def_type}to{args.ref_type}/matchbyanchor"
    ...
    print(f"用于计算映射位置的应变: {args.inter_strain} (def: {args.def_type} - ref: {args.ref_type})，用于计算应变的初始样本高度")  # 阶段间应变，用于映射位置

    search_radius = args.multiple * args.d50  # 搜索候选颗粒时用的范围
    print(f"Using search radius: {search_radius} (multiple: {args.multiple} * d50: {args.d50})")
    ...
    if args.w == 0.75:
        ...
        print(f"按照应变计算权重: {w_strain} (def: {args.def_type})")
        weightUD = w_strain / (w_strain + 0.05)  
        weightLD = 1 - weightUD
    print(f"Using weights: UD={weightUD}, LD={weightLD} (w={args.w})")
    print(f"总迭代轮数: {args.n_rounds} (含初始匹配及第2轮)")
    ...
    ref_sample_height = init_match_data['ref_sample_height'].item()  # 只有在load1toorigin中名称才是sample_height，其余都为ref_sample_height
    ...
    # ========== 第二轮：4d50内锚点，至少K个，否则跳过 ==========
    print(f"开始第二轮匹配...")
    radius_4d50 = 4 * args.d50  # 第二轮匹配时用于搜索计算LD的锚点的半径
    ...
    # 多轮迭代（第3~n_rounds轮）
    round_results, round_skipped = multi_round_matching(
        n_rounds=args.n_rounds,
        pending_particles=pending_particles,
        candidate_info=candidate_info,
        anchor_data=merge_anchors_with_matched(anchor_data, {r['pending']: r['candidate'] for r in results_round2 if r['matched']}, def_mapping, ref_mapping),
        def_mapping=def_mapping,
        ref_mapping=ref_mapping,
        args=args,
        weightUD=weightUD,
        weightLD=weightLD,
        results_round2=results_round2,
        skipped_pending2=skipped_pending2
    )

    # ========== 保存结果 ==========
    os.makedirs(args.output_dir, exist_ok=True)
    # 保存第二轮
    output_file2 = os.path.join(args.output_dir, f'matchbyanchor2.csv')
    # 第二轮无需changed列
    ...
    # 保存最后一轮
    output_file_last = os.path.join(args.output_dir, f'matchbyanchor{args.n_rounds}.csv')
    output_file_prev = os.path.join(args.output_dir, f'matchbyanchor{args.n_rounds-1}.csv')
    best2 = {r['pending']: r['candidate'] for r in results_round2 if r['matched']}
    if len(round_results) >= 2:
        save_match_results_csv(round_results[-2], output_file_prev, best2=best2)
    save_match_results_csv(round_results[-1], output_file_last, best2=best2)
    ...
    with open(os.path.join(args.output_dir, 'pending_no_candidate.csv'), 'w', newline='') as f:
        ...
        for p in no_candidate_pending:
            writer.writerow([p])
    save_skipped_pending(skipped_pending2, os.path.join(args.output_dir, 'pending_skipped_anchor_round2.csv'))
    for i, skipped in enumerate(round_skipped):
        save_skipped_pending(skipped, os.path.join(args.output_dir, f'pending_skipped_anchor_round{3+i}.csv'))
